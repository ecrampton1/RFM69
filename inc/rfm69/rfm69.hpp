...
private:
	...
	{
		waitForModeReady();
		if( rfm69_comm::writePacket(header, length, buf) < 0) {
			return false;
		}
		enablePacketSentIrq();
		enableTx();
	...
	static void waitForAck()
	{
		int j = 0;
#ifdef __arm__
		while(false == _irq::read() && ++j < ACK_TIMEOUT) {
			_sys::delayInMs(1);
		}
#else	
...
template< class _spi, class _sys,  class _cs, class _irq, CarrierFrequency _freq,  class _uart>
volatile bool Rfm69<_spi, _sys, _cs, _irq, _freq,  _uart>::mRxMode = false;
template< class _spi, class _sys,  class _cs, class _irq, CarrierFrequency _freq,  class _uart>
